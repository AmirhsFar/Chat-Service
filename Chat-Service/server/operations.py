...
async def check_pv_chat_existance(
    ...
    This function looks for existing private (PV) chat rooms between the
    requester and the addressed user. It searches both users' chat room 
    sessions and cross-references them to find a mutual private chat room.

    Args:
        requester_user_id (str): The unique identifier of the user 
            initiating the chat.
        addressed_user_id (str): The unique identifier of the user 
            being invited.
...
async def create_pv_chat(
    ...
    It first checks whether the two users are the same, raising an error 
    if the requester and addressed user IDs are identical. It also checks 
    for the existence of an existing private chat session between the two 
    users, raising an error if one is already present.

    Args:
        requester_user_id (str): The unique identifier of the user 
            who initiated the chat.
        addressed_user_id (str): The unique identifier of the user 
            being invited to the chat.
